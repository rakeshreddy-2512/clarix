...
function extractInnerHtml(html: string): string {
    ...
    if (match) { console.log("✅ Found pageSanitizer content"); return unescapeHtml(match[1]); }
    const innerMatch = html.match(/\.innerHTML\s*=\s*pageSanitizer\.sanitize\('([\s\S]+?)'\)/);
    if (innerMatch) { console.log("✅ Found innerHTML content"); return unescapeHtml(innerMatch[1]); }
...
export function parseAttendance(html: string): AttendanceCourse[] {
    ...
    $inner("table").each((_, table) => {
        ...
        if (headerText.includes("attn") && headerText.includes("slot") && headerText.includes("course") && headerText.includes("category") && headerText.includes("faculty")) {
    ...
    if (!attendanceTable) {
        ...
        $inner("table").each((i, table) => { console.log(`  Table ${i} header: ${$inner(table).find("tr:first-child").text().trim().substring(0, 80)}`); });
        return courses;
    }
    ...
    $inner(attendanceTable).find("tr").each((i, row) => {
        ...
        courses.push({ code, title, faculty, category: category === "Practical" ? "Practical" : "Theory", slot, room, totalClasses: 0, attended: 0, absent: 0, percentage });
    });

    console.log(`✅ Total parsed: ${courses.length} courses`);
    return courses;
}