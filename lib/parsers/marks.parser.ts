...
export function parseMarks(html: string): MarkCourse[] {
    ...
    $("table").each((_, table) => {
        const headerText = $(table).find("tr:first-child").text().toLowerCase();
        if (headerText.includes("test performance") || headerText.includes("course type")) marksTable = $(table);
    });

    if (!marksTable) { console.warn("⚠️ Could not find marks table"); return courses; }
    ...
    $(marksTable).find("tr").each((i, row) => {
        ...
        if (code && tests.length > 0) courses.push({ code, title, type: type === "Practical" ? "Practical" : "Theory", tests });
    });
