...
export default function CalendarPage() {
    const { data, loading } = useFetchWithCache<PlannerData>(
    ...
    const months = data ? [...new Set(Object.keys(data.map).map(d => d.substring(0, 7)))].sort() : [];
    const activeMonth = selectedMonth || (months.includes(today) ? today : months[months.length - 1] || "");

    if (loading) return <LoadingScreen />;

    const currentMonthDays = data
        ? Object.values(data.map).filter(d => d.date.startsWith(activeMonth)).sort((a, b) => a.date.localeCompare(b.date))
        : [];
                    ...
                    {currentMonthDays.map((day, i) => {
                        ...
                        return (
                            <div key={day.date} style={{
                                display: "grid", gridTemplateColumns: "60px 60px 1fr 80px",
                                padding: "12px 16px", alignItems: "center",
                                background: isToday ? "#eff6ff" : i % 2 === 0 ? "#ffffff" : "#f8fafc",
                                borderTop: "1px solid #f1f5f9",
                                borderLeft: isToday ? "3px solid #1d4ed8" : "3px solid transparent",
                            }}>
                                <div style={{ fontSize: 14, fontWeight: isToday ? 800 : 600, color: isToday ? "#1d4ed8" : isWeekend ? "#94a3b8" : "#0f172a" }}>
                                    {String(dateNum).padStart(2, "0")}
                                </div>
                                <div style={{ fontSize: 13, fontWeight: 600, color: isWeekend ? "#94a3b8" : "#64748b" }}>{dayOfWeek}</div>
                                <div style={{ fontSize: 13, fontWeight: day.note ? 600 : 400, color: isHoliday ? "#dc2626" : day.note ? "#0f172a" : "#94a3b8" }}>
                                    {day.note || (isWeekend ? "Weekend" : day.dayOrder ? "Regular Classes" : "-")}
                                </div>
                                <div>
                                    {day.dayOrder ? (
                                        <span style={{ fontSize: 12, fontWeight: 700, padding: "3px 10px", borderRadius: 20, background: doColors?.bg, color: doColors?.color, border: `1px solid ${doColors?.border}` }}>
                                            DO {day.dayOrder}
                                        </span>
                                    ) : (
