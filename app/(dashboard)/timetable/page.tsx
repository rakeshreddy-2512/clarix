...
async function drawTimetable(doc: any, timetable: Timetable, batch: number, section: string, profile: Student | null) {
    ...
    doc.text("Generated by Clarix — Academia Tracker", pageW / 2, pageH - 1, { align: "center" });
}
...
export default function TimetablePage() {
    ...
    const activeTimetable = editMode
        ...
        : (hasCustom && editedTimetable ? editedTimetable : originalTimetable);
    const slots = activeTimetable[selectedDay] ?? [];
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            await fetch("/api/timetable/custom", {
                ...
                body: JSON.stringify({ timetable: editedTimetable }),
            });
            setHasCustom(true);
            setEditMode(false);
            setSelectedCourse(null);
    ...
    const handleRevert = async () => {
        if (!window.confirm("Revert to your original academia timetable? Your custom changes will be lost.")) return;
        setSaving(true);
        try {
            const token = getToken();
